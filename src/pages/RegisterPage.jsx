  ...
  const validateUserExists = async () => {
    try {
      ...
      if (response.ok) {
        ...
        if (userIdExists) { setError("El ID de usuario ya está registrado en el sistema"); return false; }
        if (emailExists) { setError("El correo electrónico ya está registrado en el sistema"); return false; }
        return true;
      }
    } catch (error) {
  ...
  return (
    <div className="login-page">
      <div className="login-box">
        ...
        {error && (
          <div style={{background:'#ffe6e6',border:'1px solid #ff6b6b',color:'#c92a2a',padding:'12px 15px',borderRadius:'6px',marginBottom:'20px',fontSize:'14px'}}>
            {error}
          </div>
        )}
        <form onSubmit={handleRegister}>
          <input type="text" value={userId} onChange={(e) => setUserId(e.target.value)} className="login-input" placeholder="User ID" required disabled={loading} />
          <input type="text" value={userName} onChange={(e) => setUserName(e.target.value)} className="login-input" placeholder="Username" required disabled={loading} />
          ...
          <div style={{ position: 'relative' }}>
            <input type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} className="login-input" placeholder="Password" required disabled={loading} style={{ paddingRight: '45px' }} />
